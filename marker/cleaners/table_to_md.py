...
class TableToMD:
    def __init__(self):
        ...
        self.table_detect_model = TableTransformerForObjectDetection.from_pretrained(
    ...
    def extract_table(self, pdf, page_num, tess_lang):
        ...
        results = self.feature_extractor.post_process_object_detection(
            outputs, threshold=0.7, target_sizes=target_size
        )[0]
        print("this is the results[boxes]")
        print(results["boxes"])
        if len(results["boxes"]) > 0:
            ...
            for index, (box, score) in enumerate(
                zip(results["boxes"], results["scores"])
            ):
                with tempfile.NamedTemporaryFile(
                    ...
                    horizontal_lines_bbox, vertical_line_bboxes = (
                        self.detect_borderlines(image_path)
                    )
                    print("ch1")
                    print(horizontal_lines_bbox, vertical_line_bboxes)
                    text_line_bbox, text_line_height = self.get_horizontal_textlines(
                        data, image
                    )
                    text_line_bbox = [
                        bbox for bbox in text_line_bbox if bbox[2] > bbox[3]
                    ]
                    _, _, _, new_horizontal_lines_bbox = (
                        self.filter_non_intersecting_lines(
                            text_line_bbox,
                            horizontal_lines_bbox,
                            horizontal_lines_height=text_line_height / 4,
                        )
                    )
                    horizontal_lines_bbox_clustered = self.cluster_horizontal_lines(
                        new_horizontal_lines_bbox, text_line_height
                    )
                    vertical_line_bboxes_clustered = self.cluster_vertical_lines(
                    ...
                    # bboxes_on_image(image_path , both_lines)
    ...
    def detect_borderlines(
        self, file_path, angle_threshold=1, vertical_slope_threshold=10
    ):
        ...
        lines = cv2.HoughLinesP(
            edges, 1, np.pi / 180, threshold=300, minLineLength=200, maxLineGap=10
        )
        ...
        if lines is not None:
            for line in lines:
                ...
                if x2 != x1:
                    ...
                    if abs(slope) < np.tan(np.radians(angle_threshold)):
                    ...
                    # Vertical line check
                    elif abs(slope) > np.tan(np.radians(90 - angle_threshold)):
                        min_x, max_x = min(x1, x2), max(x1, x2)
                        min_y, max_y = min(y1, y2), max(y1, y2)
    ...
    def filter_non_intersecting_rectangles(self, horizontal_rects, vertical_rects):
        ...
        intersecting_horizontal_lines = [
            original_horizontal_rects[i] for i in intersecting_horizontal_indexes
        ]
        non_intersecting_horizontal_lines = [
            original_horizontal_rects[i]
            for i in range(len(original_horizontal_rects))
            if i not in intersecting_horizontal_indexes
        ]
        intersecting_vertical_lines = [
            original_vertical_rects[j] for j in intersecting_vertical_indexes
        ]
        non_intersecting_vertical_lines = [
            original_vertical_rects[j]
            for j in range(len(original_vertical_rects))
    ...
    def draw_bboxes_on_image(self, image_path, bboxes):
        ...
        for bbox in bboxes:
            x, y, w, h = bbox
            cv2.rectangle(
                image_with_bboxes, (x, y), (x + w, y + h), (0, 255, 0), 2
            )  # Draw the rectangle
    ...
    def cluster_vertical_lines(self, vertical_lines_bbox, v):
        ...
        for cluster in clusters:
            ...
            merged_line = [
                ...
                1,
                max_y - min_y,
            ]  # Using 1 as the width for the new line
            merged_lines.append(merged_line)
    ...
    def get_df_from_lines(
        self, horizontal_lines_bbox_clustered_filt, vertical_line_bboxes_clustered, data
    ):
    ...
    def overlap(rect1, rect2):
        return TableToMD.range_overlap(
            rect1.left, rect1.right, rect2.left, rect2.right
        ) and TableToMD.range_overlap(rect1.top, rect1.bottom, rect2.top, rect2.bottom)
    ...
    def filter_non_intersecting_lines(
        ...
        if not pd.isnull(vertical_lines_width):
            ...
            vertical_lines = [
                (x, y, vertical_lines_width, h) for x, y, w, h in vertical_lines
            ]

        if not pd.isnull(horizontal_lines_height):
            horizontal_lines_height = int(np.ceil(horizontal_lines_height))
            horizontal_lines = [
                (x, y, w, horizontal_lines_height) for x, y, w, h in horizontal_lines
            ]

        horizontal_rects = [
            self.Rectangle(x, y, w, h) for x, y, w, h in horizontal_lines
        ]
